...
// A fully illustrated office scene (drawn in SVG) with 5 hazards hidden in
// plausible visual locations. The player must visually identify and click on
// them — nothing is pre-labeled on screen, so it is a genuine spot-the-hazard
// challenge rather than a button list.
const HAZARDS = [
    ...
    <div>
      ...
      <div className="relative rounded-xl overflow-hidden border border-nestle-blue/40" style={{ aspectRatio: '16/10' }}>
        <svg viewBox="0 0 100 62" className="w-full h-full" preserveAspectRatio="xMidYMid slice">
          ...
          {/* HAZARD 1: wet floor patch (visual only, no label) */}
          ...
          {/* HAZARD 3: boxes blocking exit door */}
          ...
          {/* HAZARD 4: overloaded socket with multiple plugs */}
          ...
          {/* HAZARD 5: precariously stacked boxes on shelf */}
        ...
        {/* Clickable hazard hit-zones (invisible until found) */}
        {HAZARDS.map(h => (
