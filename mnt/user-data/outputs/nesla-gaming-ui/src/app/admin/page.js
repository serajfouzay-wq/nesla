    ...
    <div className="min-h-screen">
      ...
      <main className="pt-20 pb-16 px-4 max-w-6xl mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-black text-white">🔒 {lang === 'en' ? 'Admin Dashboard' : 'Papan Pemuka Admin'}</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="card-hud text-center"><p className="text-3xl font-black text-blue-400">{rows.length}</p><p className="text-xs text-gray-400 mt-1">{lang === 'en' ? 'Teams registered' : 'Pasukan berdaftar'}</p></div>
          <div className="card-hud text-center"><p className="text-3xl font-black" style={{ color:'#00FF88' }}>{totalSubmissions}</p><p className="text-xs text-gray-400 mt-1">{lang === 'en' ? 'Module submissions' : 'Penyerahan modul'}</p></div>
          <div className="card-hud text-center"><p className="text-3xl font-black text-yellow-400">{leader?.total ?? 0}</p><p className="text-xs text-gray-400 mt-1">{lang === 'en' ? 'Top score' : 'Markah teratas'}</p></div>
          <div className="card-hud text-center"><p className="text-xl font-black text-orange-400 truncate">{leader?.groupName ?? '—'}</p><p className="text-xs text-gray-400 mt-1">{lang === 'en' ? 'Current leader' : 'Pemimpin semasa'}</p></div>
        </div>

        <div className="flex gap-2 mb-4">
          <button onClick={() => setView('board')} className="px-4 py-2 rounded-xl text-sm font-semibold transition-colors"
            style={view === 'board' ? { background:'#E2001A', color:'white' } : { background:'rgba(26,58,107,0.4)', color:'#94A3B8' }}>🏆 {lang === 'en' ? 'Leaderboard' : 'Papan Markah'}</button>
          <button onClick={() => setView('detail')} className="px-4 py-2 rounded-xl text-sm font-semibold transition-colors"
            style={view === 'detail' ? { background:'#E2001A', color:'white' } : { background:'rgba(26,58,107,0.4)', color:'#94A3B8' }}>📋 {lang === 'en' ? 'Module Breakdown' : 'Pecahan Modul'}</button>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {rows.map((row, i) => (
              <div key={row.id} className="rounded-xl p-4 flex items-center gap-4"
                style={{
                  background: i === 0 ? 'rgba(255,215,0,0.08)' : i === 1 ? 'rgba(192,192,192,0.05)' : i === 2 ? 'rgba(205,127,50,0.07)' : 'rgba(10,22,40,0.7)',
                  border: `1px solid ${i === 0 ? 'rgba(255,215,0,0.35)' : i === 1 ? 'rgba(192,192,192,0.3)' : i === 2 ? 'rgba(205,127,50,0.35)' : 'rgba(26,58,107,0.5)'}`,
                }}>
                <div className="w-10 text-center text-xl font-black">{i < 3 ? MEDALS[i] : <span className="text-gray-500 text-sm">{i+1}</span>}</div>
                <div className="flex-1 min-w-0"><p className="font-bold truncate text-white">{row.groupName}</p><p className="text-xs text-gray-500 truncate">{row.players.join(' · ')}</p></div>
                <div className="flex items-center gap-1 flex-wrap justify-end">
                  {MODULES.map(m => (
                    <div key={m.slug} title={m.en} className="w-7 h-7 rounded-lg flex items-center justify-center text-xs"
                      style={{ background: row.modules[m.slug] ? 'rgba(0,255,136,0.1)' : 'rgba(26,58,107,0.2)', border:`1px solid ${row.modules[m.slug] ? 'rgba(0,255,136,0.3)' : 'rgba(26,58,107,0.3)'}`, opacity: row.modules[m.slug] ? 1 : 0.3 }}>{m.icon}</div>
                  ))}
                </div>
          ...
          <div className="overflow-x-auto card-hud">
            <table className="w-full text-sm">
              <thead><tr style={{ borderBottom:'1px solid rgba(26,58,107,0.5)' }}>
                <th className="text-left py-3 px-2 text-gray-400">{lang === 'en' ? 'Team' : 'Pasukan'}</th>
                {MODULES.map(m => <th key={m.slug} className="text-center py-3 px-2 text-gray-400" title={m.en}>{m.icon}</th>)}
                <th className="text-right py-3 px-2 text-gray-400">{lang === 'en' ? 'Total' : 'Jumlah'}</th>
              </tr></thead>
              <tbody>
                {rows.map((row, i) => (
                  <tr key={row.id} style={{ borderBottom:'1px solid rgba(26,58,107,0.25)' }}>
                    <td className="py-3 px-2"><div className="flex items-center gap-2"><span>{i < 3 ? MEDALS[i] : ''}</span><div><p className="font-semibold text-white">{row.groupName}</p><p className="text-xs text-gray-500">{row.players.join(', ')}</p></div></div></td>
                    {MODULES.map(m => (
                      <td key={m.slug} className="text-center py-3 px-2">
                        {row.modules[m.slug] ? <div><p className="font-bold" style={{ color:'#00FF88' }}>{row.modules[m.slug].score}</p><p className="text-xs text-gray-600">/{row.modules[m.slug].max}</p></div> : <span className="text-gray-700">—</span>}
                      </td>
                    ))}
                    <td className="text-right py-3 px-2"><span className="font-black text-lg text-white">{row.total}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
